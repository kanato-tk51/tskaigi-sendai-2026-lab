...
M0 の npm lifecycle marker-only spike は実行を試みたが、Docker CLI 不在のため全5条件が **Inconclusive** であり、lifecycle invocation は未実測のままである。これ以外の route/profile は実験を行っておらず、observed invocation count と observed result は **未実測** である。
...
|---|---|---|---|---|---|---|---|---|---|
| `npm12-unapproved-p` / `npm12-unapproved-c` | TIME, COUNT, CTX | yes、ただし hook 起動時のみ | not applicable | hook 0回なので access event なし | 同左 | 未実測（M0 infrastructure Inconclusive） | Docker CLI 不在のため scenario 未実行 | `results/examples/m0-npm12/summary.json`, `docs/spike-npm12.md` | npm 12 help、image digest、marker、approval、lockfile は未観測。0回と解釈しない。profile reach は M0 対象外 |
| `npm12-approved-control-p` / `npm12-approved-control-c` | 全項目（API除く） | yes | not applicable | hook 1回、ENV/FILE/WRITE/NET/CHILD success | hook 1回、ENV absent、FILE/WRITE/NET/CHILD denied を期待 | 未実測（M0 infrastructure Inconclusive） | Docker CLI 不在のため approved rebuild / disabled / reinstall / ci は未実行 | `results/examples/m0-npm12/summary.json`, `docs/spike-npm12.md` | `allowScripts` 表現と marker count は未観測。M0 は marker-only で capability/profile 比較を行わない。child denial は M4対象 |
