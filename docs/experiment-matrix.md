...
| `baseline-vite-p` / `baseline-vite-c` | Vite plugin | harness | explicit | p / c | probe plugin なし、entry/marker module 各1件、build mode | `route-invocation: 0` | 未実測 | fixture source hash 不変 | 未実測 | 未生成: `results/runs/<run-id>/` | 通常 build artifact は生成されるため pristine expected artifact と比較 |
...
| `vite-observe-p` / `vite-observe-c` | Vite plugin | module-evaluation; tool-initialization; transform-hook | configured; automatic | p / c | build mode、marker module 1件だけを計数、cache/watch off | `module-evaluation: 1; initialization: 1; target-transform: 1` |
| `vite-api-p` / `vite-api-c` | Vite plugin | module-evaluation; tool-initialization; transform-hook; generate-bundle-hook | configured; automatic | p / c | transform marker 1件、emitted artifact 1件、fresh output | `module/init: 1; target-transform: 1; generateBundle: 1; intended API changes: 2` |
...
`eslint` / `vite` のconfigured loadとautomatic callbackは、一つのuser command内で連続していても別triggerとしてeventに残す。明示fix optionは`explicit`とする。Vitest setupはmodule evaluationとsetup-file semanticsを別phaseにする。Codegenは`explicit`なCLI起動でありautomaticと表現しない。
