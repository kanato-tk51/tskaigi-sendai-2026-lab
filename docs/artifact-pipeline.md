...
Status: **full-lab design deferred; presentation subset active**. The active
delivery contract is the five-step [presentation MVP artifact demo](presentation-scope.md#minimum-artifact-demo).
...
- Dependency acquisitionはprobe/build/verify/deployと分離したonline-capable phaseである。
  Project humanがregistry、固定lockfile/dependency scope、credential-empty disposable configを
  明示的に承認した場合だけ外部networkを使用できる。暗黙の通信や任意hostへのfallbackは行わない。
- Acquisition後はdependency bytesをlockfileとdigestでdeclared inputへ固定する。
  Build、verify、deployは外部registryへfallbackせず、不足時はpreflight failureとする。
...
Online acquisitionを使用した場合、再現手順はそのphaseと取得identityを明記し、offline buildと
混同しない。発表demoは既存lockfile/toolchainを優先し、新しい汎用bundle配布機構を要求しない。
...
Full-lab M5では `lab artifact acquire`, `lab artifact build`, `lab artifact package`, `lab artifact verify`, `lab artifact deploy`, `lab artifact tamper-check` に相当する明示的 subcommandを将来設計できる。この汎用surfaceはpresentation MVPでは不要であり、固定fixtureに対するrepository-owned scriptまたは既存CLIのfixed operationでよい。

各 subcommand は前 stage の immutable manifest を input にし、arbitrary shell command や暗黙の install/build fallback を受け付けない。

## 設計時点の仮定と未決事項

- Online acquisitionを使う場合のexact registry/dependency scopeと取得identityの記録は実行前review pointである。Offline buildへのregistry fallbackは引き続き禁止する。
