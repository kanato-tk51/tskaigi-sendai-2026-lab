...
Offline-build recovery one-time execution-gate definition (2026-07-19; supersedes the current-next-task clause above): a non-executing recovery execution-gate candidate was prepared to bind only fixed `staged` tag, fixed `run-root`, and the existing single-inspect backend limit to one exact recovery attempt. New documents: [`prompts/m4-execution-profiles-offline-build-recovery-execution.md`](../prompts/m4-execution-profiles-offline-build-recovery-execution.md) and [`prompts/reviews/m4-execution-profiles-offline-build-recovery-gate-review.md`](../prompts/reviews/m4-execution-profiles-offline-build-recovery-gate-review.md). This definition fixes failure/digest matrix and `ownedStateDisposition: retained`, forbids retry/state mutation/build/retry/rebind, and keeps profile-binding/runtime/enforcement/Observed separate. Tracking issue: [#34](https://github.com/kanato-tk51/tskaigi-sendai-2026-lab/issues/34).

Offline-build recovery one-time execution-gate independent review (2026-07-19; supersedes the current-next-task clause above): [fresh independent read-only review](reviews/m4-execution-profiles-offline-build-recovery-gate.md) approved the fixed boundary. The gate is accepted with no blocking findings, with constraints limited to fixed recorded failure (`inconclusive/CLEANUP_FAILURE`), single inspect max 1, fixed staged tag/run-root, no-retry/no command-state mutation/no runtime-control addition, and `ownedStateDisposition: retained`. This review reran `m4:typecheck` / `m4:static` / `m4:test` / `m4:verify` / `check` and `git diff --check` successfully; no recovery execution or Docker action was performed.

Issue-34 recovery execution follow-up (2026-07-19; supersedes the current-next-task clause above): `npm run --silent m4:recovery:offline-build` was executed once under the approved one-time gate boundary. The canonical result is `validity=complete`, `primaryFailure=null`, `completedSteps=[validate-retained-state, inspect-image, validate-retained-state-after-inspect]`, `ownedStateDisposition="retained"`, and `builtImageDigest="sha256:20ba341937bfaee4fe8d1adc722aed4c7dc96d055371bf7b48ba3cd12e15e3dd"`. `run-root` and fixed staged tag remain for later gates.

Run-controls gate definition (2026-07-19; supersedes the current-next-task clause above): runtime-enforcement execution is now blocked to a one-shot `npm run --silent m4:run:controls` gate that binds fixed `builtImageDigest`, fixed permissive/constrained pair run IDs, fixed docker runtime command boundary, and `one-shot/no-retry` semantics before any observed status change. The non-executing artifacts for this gate are [`prompts/m4-execution-profiles-run-controls-gate.md`](../prompts/m4-execution-profiles-run-controls-gate.md) and [`prompts/reviews/m4-execution-profiles-run-controls-gate-review.md`](../prompts/reviews/m4-execution-profiles-run-controls-gate-review.md).

Tracking issue: [#40](m4-restart-issue-board.md). Profile-control Observed route escalation remains blocked until this gate is approved and followed by separate evidence-class boundary work.

Run-controls gate fresh independent review (2026-07-20; supersedes the current-next-task clause above): [fresh Docker-non-executing review](reviews/m4-execution-profiles-run-controls-gate.md)はrecovery trailのnon-null built-image digestとreview対象hashを再現したが、gateをB-18/B-19/B-20でblockした。Canonical `profile.json`とexact pair run IDがなく、ordinary entryはfail closed、production `FixedExecutionBackend`とexact activation bytesは未実装である。さらにcurrent pair executorはrecovered imageをconsumeせず`stage-build-context` / `doctor` / `build` / `inspect-image`を再実行するため、candidateのno-rebuild/no-rebind boundaryと矛盾する。ReviewはDocker、temporary activation、`run:controls`を実行せず、Observedを変更していない。

Run-controls exact run-ID decision (2026-07-20): project humanの継続指示によりpermissive=`m4-profile-control-p-20260720-01`、constrained=`m4-profile-control-c-20260720-01`を固定した。この2値はdistinct immutable inputであり、remediationまたはexecution workerが変更しない。

Run-controls B-18/B-19/B-20 remediation implementation update (2026-07-20; supersedes the current-next-task clause above): [`prompts/m4-execution-profiles-run-controls-remediation.md`](../prompts/m4-execution-profiles-run-controls-remediation.md)はrecovered digest、exact pair run ID、canonical `profile.json` bytes、derived run roots/container namesを固定した。Production pathはbuild-first executorから分離したexisting-image executorだけを使い、通常entry/package rootから到達不能なfixed control host backendへbindする。Backendはpair run-root absenceを両方確認してからprivate manifest/result/scratch/config stateを作り、fixed create/inspect/start/transfer/removeだけを許す。Exact temporary activation/restoration bytesとfresh re-review promptも記録した。Static/unit実装はDockerを実行せず、runtime enforcement/Observedを変更していない。

Run-controls remediation fresh independent re-review (2026-07-20; supersedes the current-next-task clause above): [fresh Docker-non-executing re-review](reviews/m4-execution-profiles-run-controls-remediation.md)はcanonical profile/digest/run binding、両run-root absence gate、existing-image/no-rebuild dependency、production host backend、ordinary import非到達性、exact activation/restorationを再現し、B-18/B-19/B-20をstatic/unit境界でclosureした。Reviewは21 files / 223 testsの`m4:verify`と103 files / 711 testsのroot `check`を再実行し、temporary activation、Docker、`run:controls`、retained build state access、Observed更新を行っていない。更新済みexecution gateはreviewed 67-file aggregate、fixed digest/run roots/container names、activation/restoration hashes、one-shot/no-retry commandを固定して承認済みである。

Run-controls one-time execution follow-up (2026-07-20; supersedes the current-next-task clause above): fresh workerはreview済み67-file aggregateとcritical source/profile/activation/restoration hashを再現し、両exact run rootの不在を確認した後、`continue-repository-work` standing authorizationを使用して`npm run --silent m4:run:controls`をちょうど1回実行した。これは別human reviewを意味しない。Commandはexit 1で、canonical pair/permissive/constrained resultはいずれも`inconclusive / COMMAND_FAILURE`、completed stepなし、comparison/completionなしである。両exact run rootは作成され、各rootにはcanonical input manifestと空のhost/container-result/scratch directoryだけが残った。Retry、追加Docker command、retained build state accessは行っていない。Ordinary source/compiled outputはreview済みhashへ復元済みである。このone-shotはruntime enforcement、profile-control Observed、adapter/experiment-matrix route Observed、presentation evidenceを確立・変更しない。

Issue #41 evidence-class alignment closure (2026-07-20; supersedes the current-next-task clause above): [`M4 execution profiles`](m4-execution-profiles.md#profile-control-observed-acceptance-boundary)はprofile-control Observedを、両fixed runが`complete`でimmutable input、full host inspection、canonical container evidence、completion、Expected/Observed comparisonを全て持つ場合だけに限定した。実際に試行された`failure`はcomplete evidence chain内の有効な観測になり得る。Expected mismatchはcomplete run内のObserved mismatchとして保持するが、期待したenforcement成立とは扱わない。`inconclusive`、未試行、manifest skip、harness/command/inspection/transfer/evidence/cleanup failure、static/unitまたはhost inspection単独はObservedにならない。Complete profile-control evidenceもcontrol fixture専用であり、別review済みadapter/profile scenarioとcomplete route runなしにexperiment-matrix route Observedへ昇格しない。

The exhausted pairは全resultが`inconclusive / COMMAND_FAILURE`、completed stepなし、inspection/evidence/completion/comparisonなしであるため、runtime enforcement、profile-control Observed、adapter/experiment-matrix route Observed、presentation evidenceを確立しない。Prior execution workerはreview済みexact command 1回にstanding authorizationを使用したが、別human reviewを意味しない。Pairを再実行せず、run IDを置換せず、`experiment-matrix.md`を変更していない。Issue #41と親#37はこの明示的Inconclusive limitationでローカル台帳上完了した。
...
- The resumed M4 recovery/control continuation is complete at its bounded
  one-shot scope. Recovery produced the fixed built-image digest, while the
  later fixed control pair exhausted as `inconclusive / COMMAND_FAILURE`
  before any completed step or control evidence. It is not runtime enforcement
  or profile/route Observed, and it is not retryable.
