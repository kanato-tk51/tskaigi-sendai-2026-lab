...
codegen exact context, separated bindings, sanitized projection, and fixed
runner source now have static/unit coverage. The current next task is its exact
staging assembly; no selected profile run or matrix Observed exists yet.
