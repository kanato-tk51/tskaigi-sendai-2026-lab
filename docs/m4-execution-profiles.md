...
Presentation-MVP freeze update (2026-07-18; supersedes every M4 next-task clause above for scheduling): [ADR-0002](decisions/0002-prioritize-presentation-mvp.md)はB-16/B-17を既知の未解決事項としてhigh-assurance recovery trackを凍結した。通常entryはfail closedのまま維持し、MVPではrecovery remediation、retained tag inspect/retry、state repair/delete、追加execution gateへ進まない。M4 profile-controlはoptional diagnostic/appendixであり、route Observedへ自動昇格しない。[Presentation evidence inventory](presentation-evidence-inventory.md)とその[独立review](reviews/presentation-evidence-inventory.md)は完了し、P1 gateはfindingなしでapprovedである。Active next taskはcodegen mappingと選定4 run境界を固定するDocker非実行のP2 contractである。M4 retained stateと通常entryはこのP2 boundaryから到達不能のままである。
