...
Presentation-MVP freeze update (2026-07-18; supersedes every M4 next-task clause above for scheduling): [ADR-0002](decisions/0002-prioritize-presentation-mvp.md)はB-16/B-17を既知の未解決事項としてhigh-assurance recovery trackを凍結した。通常entryはfail closedのまま維持し、MVPではrecovery remediation、retained tag inspect/retry、state repair/delete、追加execution gateへ進まない。M4 profile-controlはoptional diagnostic/appendixであり、route Observedへ自動昇格しない。Active next taskは[presentation evidence inventory](presentation-scope.md#delivery-sequence)であり、その後は選定したVite/codegen route pairの最小fixed runnerを別境界で扱う。
