...
Run-controls remediation architecture update (2026-07-20): recovered digestとexact pair run IDをversioned canonical `profile.json`へbindし、control execution edgeをhistorical build-first executorから分離した。`run-controls.ts`はexisting-image executorだけを構成し、production host backendは両run rootの事前absence、fixed command object/order、private manifest/result/scratch/config state、canonical transfer/host recordsへ限定する。Backend/runnerはordinary orchestratorとpackage rootから到達不能で、temporary activationは別fresh review/execution gateまで適用しない。これはstatic/unit設計証拠でありruntime enforcement/Observedではない。

Run-controls evidence-class closure (2026-07-20): the reviewed temporary edge was used once and restored, but the fixed pair stopped as `inconclusive / COMMAND_FAILURE` before any completed step, host inspection, container evidence, completion, or comparison. Canonical profiles and the existing-image/backend implementation remain Expected/configuration and static/unit evidence. Profile-control Observed requires a complete two-run evidence chain; an otherwise complete Expected mismatch remains an Observed mismatch rather than a successful-enforcement claim. Inconclusive results never cross that boundary, and even complete control evidence has no dependency edge to adapter or experiment-matrix route Observed. `experiment-matrix.md` is therefore unchanged.
